...
-   `token`: Token associated with the child account request.
...
## Child Account Table (`obrn_subscription_child`)

This table stores information about child accounts associated with subscriptions within the Oberon plugin. Each record contains details such as the subscription ID and child account ID.

-   `id`: Unique identifier for each child account entry.
-   `subscription_id`: ID of the subscription associated with the child account.
-   `child_account_id`: ID of the child account.
-   `created_at`: Timestamp indicating when the child account was associated with the subscription.
